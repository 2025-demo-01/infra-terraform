...
resource "helm_release" "karpenter" {
  ...
  set {
    ...
    value = "${var.cluster_name}-karpenter-int-queue"
  }
  set {
...
# Karpenter EC2NodeClass (공유 SG/서브넷 태그 기반)
resource "kubectl_manifest" "ec2nodeclass_ll" {
  yaml_body = <<YAML
apiVersion: karpenter.k8s.aws/v1beta1
kind: EC2NodeClass
metadata:
  name: ec2nodeclass-ll
  namespace: karpenter
spec:
  amiFamily: AL2023
...
kind: EC2NodeClass
metadata:
  name: ec2nodeclass-spot
  namespace: karpenter
spec:
  amiFamily: AL2023
  role: ${aws_iam_role.ng.name}
  subnetSelectorTerms:
    - tags: { karpenter: "enabled" }
  securityGroupSelectorTerms:
    - tags: { karpenter: "enabled" }
  tags:
    workload: "general"
...
# NodePool — 저지연(OD 우선+스팟 보조), NUMA 친화/CPU pinning 힌트 라벨, taint
...
# NodePool — 범용 워크로드(Spot 우선), 비용 최적화
